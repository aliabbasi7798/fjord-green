...
class LearnersEnsemble(object):
    ...
    def compute_gradients_and_loss(self, batch, weights=None, p=None):
        """
        ...
        for learner_id, learner in enumerate(self.learners):
            loss = learner.compute_gradients_and_loss(batch, weights=weights, p=p)
            losses.append(loss)

        return losses

    def fit_batch(self, batch, weights, p):
        """
        ...
        for learner_id, learner in enumerate(self.learners):
            ...
            if weights is not None:
                learner.fit_batch(batch=batch, weights=weights[learner_id], p=p)
            else:
                learner.fit_batch(batch=batch, weights=None, p=p)
    ...
    def fit_epochs(self, iterator, n_epochs, weights=None, p=None):
        """
        ...
        for learner_id, learner in enumerate(self.learners):
            old_params = learner.get_param_tensor()
            if weights is not None:
                learner.fit_epochs(iterator, n_epochs, weights=weights[learner_id], p=p)
            else:
                learner.fit_epochs(iterator, n_epochs, weights=None, p=p)
            params = learner.get_param_tensor()
